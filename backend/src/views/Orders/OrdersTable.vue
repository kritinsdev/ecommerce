    ...
    <div class="flex justify-between border-b-2 pb-3">
      <div class="flex items-center">
        ...
        <select @change="getOrders(null)" v-model="perPage"
          class="appearance-none relative block w-24 px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 focus:z-10 sm:text-sm">
      ...
      <div>
        <input v-model="search" @change="getOrders(null)"
          class="appearance-none relative block w-48 px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 focus:z-10 sm:text-sm"
    ...
    <table class="table-auto w-full">
      ...
      <tbody v-else>
        <tr v-for="(order, index) of orders.data">
          ...
          <td class="border-b p-2">
            {{ $filters.currencyEUR(order.total_price) }}
          </td>
          <td class="border-b p-2 max-w-[200px] whitespace-nowrap overflow-hidden text-ellipsis">
            {{ order.created_at }}
          </td>
          <td class="border-b p-2 ">
            <router-link :to="{ name: 'app.orders.view', params: { id: order.id } }" class="w-8 h-8 rounded-full text-emerald-700 border border-emerald-700 flex justify-center items-center
